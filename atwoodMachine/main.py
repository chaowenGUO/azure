...
solution = sum(key * sympy.factor(value) for key, value in sympy.collect(sympy.expand(sympy.dsolve(sympy.euler_equations(lagrangian)[0]).rhs), time, evaluate=False).items())
response += solution,
